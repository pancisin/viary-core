...
export default (options) => {
  ...
  const createDiary = ({ commit, dispatch, rootGetters }, diary) => {
    ...
    return api(rootGetters["$_settings/offlineMode"])
      .postDiary(diary)
      .then((result) => {
  ...
  const handleDiaryNoteChannel = ({ commit }, noteWs) => {
    ...
    if (mutationType == null) {
      throw new Error(
        `${noteWs.operation} operation is not implemented therefore it cannot be handled properly.`
      );
    }
  ...
  const scopeDiary = ({ commit, getters, dispatch }, { slug, scopeDate }) => {
    return new Promise((resolve, reject) => {
      ...
      const day =
        scopeDate != null ? DateTime.fromSQL(scopeDate) : DateTime.local();
      dispatch("scopeDay", { day, force: true }).then(() => {
        dispatch(
          "$_settings/updateUserPreference",
          { key: Prefs.SCOPED_DIARY, value: diary.slug },
          { root: true }
        );
        resolve(diary);
      });
  ...
  const scopeDay = ({ commit, getters, dispatch }, { day, force }) => {
    return new Promise((resolve) => {
      const weekUpdate =
        day.weekNumber !== getters.scopedDay.weekNumber || force;
      commit(types.SET_LOADING_DIARY, true);
  ...
  const loadWeekData = (
    ...
    const week = DateTime.fromObject({ weekNumber, weekYear: year });
  ...
  const loadWeekWeatherData = ({ commit, getters, dispatch, rootGetters }) => {
    ...
    return WeatherPouchApi()
      .getForecastData(cityName, date)
      .then((data) => {
  ...
  const updateDayNote = (
    ...
    return api(rootGetters["$_settings/offlineMode"])
      ...
      .then((result) => {
        ...
        commit(types.SET_LOADING_DIARY, false);
        pouch.syncUpdateNote(result.id, {
          ...result,
          date_number: ordinal,
          year
        });
  ...
  const addDayNote = (
    ...
    return api(rootGetters["$_settings/offlineMode"])
      .postNote(getters.scopedDiary.slug, {
        ...note,
        ordinal,
        year: scopedDay.year
      })
  ...
  const deleteDayNote = (
    { commit, getters, rootGetters },
    { noteId, weekNumber, year, ordinal }
  ) => {
    commit(types.SET_LOADING_DIARY, true);
    return api(rootGetters["$_settings/offlineMode"])
      .deleteNote(noteId)
